...
path_onnx = os.path.join(args['keras_path'], os.path.basename(args['model']).split('.')[0]+'.onnx')
torch.onnx.export(model, dummy_input, path_onnx, input_names=['test_input'], output_names=['test_output'])

# Convert ONNX to Keras
onnx_model = onnx.load(path_onnx)
k_model = onnx_to_keras(onnx_model, ['test_input'])

keras.models.save_model(k_model,os.path.basename(args['model']).split('.')[0]+'.h5',overwrite=True,include_optimizer=True)